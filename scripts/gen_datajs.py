...
DATA['fed'] = {
    'regime': {'label':'观望期, 鹰鸽分化','signal':'mixed','confidence':'高置信',
        'description':f'政策利率 {f2(val("ffr_up"))}%-{f2(val("ffr_lo"))}% 维持不变, 下一次行动大概率降息。缩表 (WALCL {comma(v_walcl/1000,1)}T, 周 {bp(tfm("walcl")["w"]/1000*1000, "$B")}) 持续推进, RRP 缓冲 (${f2(v_rrp2)}B) 已耗尽, 未来 QT 将直击准备金。'},
    'keySignals': [
        {'title':f'RRP 余额仅 ${f2(v_rrp2)}B','meaning':'货币市场基金可搬回美联储的钱基本耗尽, 未来缩表冲击无缓冲。','direction':'bearish'},
        {'title':f'WALCL {comma(v_walcl/1000,1)}T 持续下行','meaning':'QT 每周缩减, 是净流动性的稳定逆风。','direction':'mixed'},
        {'title':f'银行准备金 {comma(v_res/1000,2)}T','meaning':'仍在 3 万亿上方, 处于"充裕"区间, 3 万亿是关键心理位。','direction':'bullish'},
    ],
    'metrics': [
        {'label':'总资产','value':f'${comma(v_walcl/1000,2)}T','change':wk('walcl'),'dir':'down','tag':'WALCL','percentile':pct('walcl'),'signal':'bearish','meaning':'缩表持续推进','changes':wk_dict('walcl'),'sparkline':series30('walcl')},
        ...
        {'label':'国债持仓','value':f'${comma(val("treast")/1000,2)}T','change':wk('treast'),'dir':'down','tag':'TREAST','percentile':pct('treast'),'signal':'mixed','meaning':'被动缩表, 节奏可控','changes':wk_dict('treast'),'sparkline':series30('treast')},
        {'label':'MBS 持仓','value':f'${comma(val("mbst")/1000,2)}T','change':wk('mbst'),'dir':'down','tag':'MBST','percentile':pct('mbst'),'signal':'mixed','meaning':'提前还款低迷, MBS缩减慢','changes':wk_dict('mbst'),'sparkline':series30('mbst')},
        {'label':'银行准备金','value':f'${comma(v_res/1000,2)}T','change':f'+${comma(tfm("resbal")["w"]/1000,0)}B/周','dir':'up','tag':'WRESBAL','percentile':pct('resbal'),'signal':'bullish','meaning':'充裕区间','changes':wk_dict('resbal'),'sparkline':series30('resbal')},
    ...
    'trendData': [
        {'name':'美联储总资产','unit':'$B','current':f'${comma(v_walcl/1000,2)}T','changes':{k:(round(tfm("walcl")[k]/1000,1) if tfm("walcl")[k] else None) for k in ('d','w','m','h6')},'meaning':'缩表速度恒定, 净流动性的稳定逆风'},
        {'name':'银行准备金','unit':'$B','current':f'${comma(v_res/1000,2)}T','changes':{k:(round(tfm("resbal")[k]/1000,1) if tfm("resbal")[k] else None) for k in ('d','w','m','h6')},'meaning':'近月回升但半年仍低, 3万亿关口是关键'},
    ...
    'chartData': {'labels': list(range(len(series90('walcl')))), 'series': {
        '总资产': [round(x/1000,2) for x in series90('walcl')], '国债': [round(x/1000,2) for x in series90('treast')], 'MBS': [round(x/1000,2) for x in series90('mbst')]}},
    'policyTable': [
...
DATA['liquidity'] = {
    ...
    'metrics': [
        {'label':'净流动性','value':f'${comma(v_nl/1000,2)}T' if v_nl else '—','change':f'{bp(tfm("netliq")["w"]/1000*1000, "$B") if tfm("netliq")["w"] else "—"}','dir':dir_of(tfm("netliq")["w"]) if tfm("netliq")["w"] else 'neutral','tag':'NetLiq','percentile':pct('netliq'),'signal':'bearish','meaning':'WALCL−RRP−TGA','changes':{k:(bp(tfm("netliq")[k]/1000*1000, "$B") if tfm("netliq")[k] is not None else '—') for k in ('d','w','m','h6')},'sparkline':series30('netliq')},
        {'label':'美联储总资产','value':f'${comma(v_walcl/1000,2)}T','change':wk('walcl'),'dir':'down','tag':'WALCL','percentile':pct('walcl'),'signal':'bearish','meaning':'QT 第一驱动','changes':wk_dict('walcl'),'sparkline':series30('walcl')},
        ...
        {'label':'银行准备金','value':f'${comma(v_res/1000,2)}T','change':f'+${comma(tfm("resbal")["w"]/1000,0)}B/周','dir':'up','tag':'Reserves','percentile':pct('resbal'),'signal':'bullish','meaning':'充裕区间下沿','changes':wk_dict('resbal'),'sparkline':series30('resbal')},
    ...
    'trendData': [
        {'name':'净流动性','unit':'$B','current':f'${comma(v_nl/1000,2)}T' if v_nl else '—','changes':{k:(round(tfm("netliq")[k]/1000,1) if tfm("netliq")[k] is not None else None) for k in ('d','w','m','h6')},'meaning':'收缩趋势, RRP耗尽后斜率变陡'},
        ...
        {'name':'银行准备金','unit':'$B','current':f'${comma(v_res/1000,2)}T','changes':{k:(round(tfm("resbal")[k]/1000,1) if tfm("resbal")[k] is not None else None) for k in ('d','w','m','h6')},'meaning':'近月回升但半年仍低, 3万亿关键'},
    ...
    'formula': {'totalAssets':round(v_walcl/1e6,2),'rrp':round(v_rrpn/1000,4),'tga':round(v_tgan/1000,4) if v_tgan else 0,'netLiquidity':round(v_nl/1e6,2) if v_nl else 0,
        'components':[
    ...
    'chartData': {'labels': list(range(len(series90('netliq') if v_nl else series90('walcl')))), 'series': {
        ...
        '准备金': [round(x/1000,2) for x in series90('resbal')],
        'TGA': [round(x/1000,2) for x in series90('tga')] if v_tgan else []}},
    'weeklyChanges': [
        {'component':'美联储总资产 (WALCL)','current':f'${comma(v_walcl/1000,2)}T','weekChange':cell('walcl','w'),'monthChange':cell('walcl','m'),'source':'Fed H.4.1','signal':'bearish'},
        ...
        {'component':'银行准备金 (WRESBAL)','current':f'${comma(v_res/1000,2)}T','weekChange':cell('resbal','w'),'monthChange':cell('resbal','m'),'source':'Fed H.4.1','signal':'bullish'},
        {'component':'净流动性(计算值)','current':f'${comma(v_nl/1000,2)}T' if v_nl else '—','weekChange':(bp(tfm("netliq")["w"]/1000*1000,"$B") if tfm("netliq")["w"] else '—'),'monthChange':(bp(tfm("netliq")["m"]/1000*1000,"$B") if tfm("netliq")["m"] else '—'),'source':'计算','signal':'bearish'},
    ],
